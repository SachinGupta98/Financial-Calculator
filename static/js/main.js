...
// ── CSRF helper: reads the CSRF token from the meta tag and injects it into fetch calls
...
async function deleteExpense(id) {
    await csrfFetch(`/api/expense/${id}`, { method: 'DELETE' });
    await fetchAppData();
}
...
async function deleteAsset(id) {
    if (confirm("Are you sure you want to delete this asset?")) {
        await csrfFetch(`/api/asset/${id}`, { method: 'DELETE' });
        await fetchAppData();
    }
}
...
async function deleteGoal(id) {
    if (confirm("Are you sure you want to delete this goal?")) {
        await csrfFetch(`/api/goal/${id}`, { method: 'DELETE' });
        await fetchAppData();
    }
}
...
// Currency
let exchangeRates = {};   // Always stored as rates relative to USD (USD = 1)
let ratesLastUpdated = null;

// All 20 supported currencies with labels for the rate table
...
// Accurate fallback rates (USD base, April 2025 approximate mid-market rates)
...
async function fetchRates(isManual = false) {
    const loadEl = document.getElementById('curr-loading');
    ...
    try {
        // ── Tier 1: @fawazahmed0/currency-api (jsDelivr CDN)
        //    Updated multiple times per day, sourced from ECB + 30 providers.
        //    Most accurate free option available.
        const today = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
        const res = await fetch(
            `https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@${today}/v1/currencies/usd.json`
        );
        if (!res.ok) throw new Error('fawazahmed0 API failed');
        const data = await res.json();
        if (!data.usd) throw new Error('No usd key in response');
        exchangeRates = { 'USD': 1 };
        Object.keys(CURRENCY_LABELS).forEach(code => {
            const lc = code.toLowerCase();
            if (data.usd[lc] != null) exchangeRates[code] = data.usd[lc];
        });
        source = 'Live · fawazahmed0 (ECB + 30 sources)';
    } catch (e1) {
        try {
            // ── Tier 2: open.er-api.com (daily updates)
            const res2 = await fetch('https://open.er-api.com/v6/latest/USD');
            if (!res2.ok) throw new Error('open.er-api failed');
            const data2 = await res2.json();
            if (data2.result !== 'success') throw new Error('Bad result');
            exchangeRates = { 'USD': 1 };
            Object.keys(CURRENCY_LABELS).forEach(code => {
                if (data2.rates[code] != null) exchangeRates[code] = data2.rates[code];
            });
            source = 'Daily · open.er-api.com';
        } catch (e2) {
            try {
                // ── Tier 3: Frankfurter (ECB data, business days)
                const res3 = await fetch('https://api.frankfurter.app/latest?from=USD');
                const data3 = await res3.json();
                exchangeRates = { 'USD': 1, ...data3.rates };
                // Frankfurter missing: AED, SAR, MYR, THB, KRW — fill from fallback
                Object.keys(CURRENCY_LABELS).forEach(code => {
                    if (!exchangeRates[code]) exchangeRates[code] = FALLBACK_RATES[code];
                });
                source = 'ECB · frankfurter.app';
            } catch (e3) {
                // ── Tier 4: hardcoded fallback
                exchangeRates = { ...FALLBACK_RATES };
                source = 'Offline cache (mid-market Apr 2025)';
            }
        }
    }
    ...
    ratesLastUpdated = new Date().toLocaleTimeString('en-IN', {
    ...
    const updEl = document.getElementById('curr-last-updated');
    if (updEl) {
        updEl.innerHTML = `<span class="inline-flex items-center gap-1.5">
            <span class="w-1.5 h-1.5 rounded-full ${source.startsWith('Offline') ? 'bg-orange-400' : 'bg-emerald-400'}"></span>
            ${source} &nbsp;·&nbsp; ${ratesLastUpdated}
        </span>`;
    }

    if (isManual) {
        // Brief flash on the result to confirm refresh
        const resultEl = document.getElementById('curr-result');
        if (resultEl) {
            resultEl.style.transition = 'color 0.15s';
            resultEl.style.color = '#10b981';
            setTimeout(() => { resultEl.style.color = ''; }, 600);
        }
    }
}
...
function calculateCurrency() {
    ...
    // Format result — use more decimals for small currencies (JPY, KRW)
    const decimals = ['JPY', 'KRW', 'IDR'].includes(to) ? 0 : 2;
    ...
    document.getElementById('curr-result').textContent = `${formatted} ${to}`;
    document.getElementById('curr-rate-info').textContent =
        `1 ${from} = ${unitFormatted} ${to}`;

    // Update rate base label in table header
    const baseLabel = document.getElementById('rate-table-base');
    if (baseLabel) baseLabel.textContent = from;

    // Refresh rate table if visible
    if (document.getElementById('rate-table-wrap') &&
        !document.getElementById('rate-table-wrap').classList.contains('hidden')) {
        renderRateTable();
    }
}
...
let rateTableVisible = false;
function toggleRateTable() {
    const wrap = document.getElementById('rate-table-wrap');
    const label = document.getElementById('rate-table-toggle-label');
    rateTableVisible = !rateTableVisible;
    wrap.classList.toggle('hidden', !rateTableVisible);
    label.textContent = rateTableVisible ? 'Hide rates table' : 'Show all rates vs selected currency';
    if (rateTableVisible) renderRateTable();
}
...
function renderRateTable() {
    ...
    tbody.innerHTML = Object.entries(CURRENCY_LABELS).map(([code, label]) => {
        if (code === from) return '';
        const rate = exchangeRates[code] || FALLBACK_RATES[code] || 0;
        const crossRate = rate / fromRate;
        const decimals = ['JPY', 'KRW'].includes(code) ? 2 : 4;
        const formatted = crossRate.toLocaleString('en-US', {
            minimumFractionDigits: decimals, maximumFractionDigits: decimals
        });
        return `<tr class="hover:bg-gray-900/30 transition-colors">
            <td class="px-4 py-2.5 text-gray-300">${label}</td>
            <td class="px-4 py-2.5 text-right font-mono text-gray-200 font-medium">${formatted}</td>
        </tr>`;
    }).join('');
}
...
// GST
function setGSTRate(rate, event) {
...
async function importCSV(inputEl) {
    ...
    try {
        const res = await csrfFetch('/api/import/expenses', { method: 'POST', body: formData });
        const data = await res.json();
        if (res.ok) {
...
async function scanReceipt(inputEl) {
    ...
    try {
        const res = await csrfFetch('/api/scan-receipt', { method: 'POST', body: formData });
        const data = await res.json();
...
async function saveCustomCategory() {
    ...
    if (!name) return;
    await csrfFetch('/api/categories', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, emoji, color }) });
...
let _isRenderingFiltered = false; // Guard against infinite recursion
window.renderExpenseTracker = function (filteredList) {
    if (filteredList !== undefined) {
        ...
        _isRenderingFiltered = true;
        appData.expenses = filteredList;
        _renderExpOrig();
        appData.expenses = orig;
        _isRenderingFiltered = false;
    } else if (!_isRenderingFiltered) {
        _renderExpOrig();
        applyMonthFilter();  // Apply active filter after full render
    } else {
        _renderExpOrig(); // Called during filtered render — just render, no re-filter
    }
};
...
async function saveSpendingLimit() {
    ...
    if (!cat || !limit) return;
    await csrfFetch('/api/limits', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ category: cat, limit }) });
...
async function deleteLimit(id) {
    await csrfFetch(`/api/limits/${id}`, { method: 'DELETE' });
    await fetchAppData();
    renderLimitsList();
...
async function saveRecurring() {
    ...
    await csrfFetch('/api/recurring', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ description: desc, amount: amt, category: cat, day_of_month: day }) });
...
async function deleteRecurring(id) {
    await csrfFetch(`/api/recurring/${id}`, { method: 'DELETE' });
    await fetchAppData();
    renderRecurringList();
}
...
async function autoLogRecurring() {
    try {
        const res = await csrfFetch('/api/recurring/log-due', { method: 'POST' });
        const data = await res.json();
...
async function parseSMS() {
    ...
    try {
        const res = await csrfFetch('/api/parse-sms', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sms }) });
        const data = await res.json();
        _smsResult = data;
...
function startVoiceExpense() {
    ...
    voiceRecognition.onresult = async (event) => {
        ...
        try {
            const res = await csrfFetch('/api/parse-voice-expense', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    voiceRecognition.onerror = (event) => {
        ...
        let errorMsg = `Microphone Error: ${event.error}. Please ensure microphone permissions are granted.`;
        if (event.error === 'not-allowed') {
            errorMsg = "Microphone access denied. Please click the icon in the address bar to allow microphone permissions.";
        } else if (event.error === 'no-speech') {
            errorMsg = "No speech detected. Please check your microphone, ensure it is not muted, and speak clearly. In Windows, check Settings > Privacy & security > Microphone and ensure 'Let desktop apps access your microphone' is enabled.";
        } else if (event.error === 'network') {
            errorMsg = "Network error occurred. The speech recognition service needs an internet connection.";
        }
...
/** Shows a styled persistent toast specifically for voice errors with longer timeout */
function showVoiceToast(title, message) {
    const existing = document.getElementById('finflap-voice-toast');
    if (existing) existing.remove();

    const toast = document.createElement('div');
    toast.id = 'finflap-voice-toast';
    toast.className = 'fixed bottom-24 md:bottom-8 left-1/2 -translate-x-1/2 z-[9999] max-w-sm w-[90vw] px-5 py-4 rounded-xl border border-red-600/50 bg-red-900/90 text-red-100 text-sm font-medium shadow-2xl backdrop-blur-sm cursor-pointer select-none';
    toast.innerHTML = `<div class="font-bold mb-1">${title}</div><div class="text-xs text-red-200 leading-relaxed">${message}</div>`;
    toast.onclick = () => toast.remove();
    document.body.appendChild(toast);

    // Auto-dismiss after 8 seconds (longer for error instructions)
    setTimeout(() => { if(toast.parentNode) { toast.style.opacity = '0'; setTimeout(() => toast.remove(), 400); } }, 8000);
}

// ─── FEATURE: MONTHLY SUMMARY EMAIL ──────────────────────────────────────────
async function sendMonthlySummary() {
    const btn = document.getElementById('summary-email-btn');
    const text = document.getElementById('summary-email-text');
    if (!btn || !text) return;
    ...
    try {
        const res = await csrfFetch('/api/send-monthly-summary', { method: 'POST' });
        const data = await res.json();
        if (res.ok) {
            text.textContent = '✅ Sent!';
            showToast(data.message || 'Monthly summary sent to your email! 📬', 'success');
            setTimeout(() => { text.textContent = 'Email Monthly Summary'; btn.disabled = false; }, 3000);
        } else {
            text.textContent = 'Email Monthly Summary';
            btn.disabled = false;
            showToast(data.error || 'Could not send email. Check mail settings.', 'error');
        }
    } catch (err) {
        text.textContent = 'Email Monthly Summary';
        btn.disabled = false;
...
// ─── TOAST HELPER (used by sendMonthlySummary and others) ────────────────────
function showToast(message, type = 'success') {
    const existing = document.getElementById('finflap-toast');
    if (existing) existing.remove();
    const colors = {
    ...
    const toast = document.createElement('div');
    toast.id = 'finflap-toast';
    toast.className = `fixed bottom-24 md:bottom-8 left-1/2 -translate-x-1/2 z-[999] px-5 py-3 rounded-xl border text-sm font-medium shadow-xl backdrop-blur-sm transition-all duration-300 ${colors[type] || colors.info}`;
    toast.textContent = message;
    document.body.appendChild(toast);
