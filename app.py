...
# ── Database: Supabase PostgreSQL if DATABASE_URL is set, else SQLite fallback ─
...
# ── Supabase-specific engine options ─────────────────────────────────────────
# Transaction Pooler (port 6543) is stateless — use NullPool to let Supabase
# manage connections. Direct connections (port 5432) use regular pool settings.
...
elif db_url and 'supabase.co' in db_url:
    app.config['SQLALCHEMY_ENGINE_OPTIONS'] = {
        'pool_pre_ping': True,
        'pool_size': 5,
        'pool_recycle': 300,
    ...
    try:
        if data.get('id'):
            ...
            if goal and goal.user_id == current_user.id:
                ...
                goal.saved_amount = float(data.get('savedAmount', 0))
        ...
        return jsonify({"error": f"Invalid goal data: {str(e)}"}), 400
...
---"""
    ...
    try:
        ...
        if amount <= 0:
            return jsonify({"error": "No valid amount found in transcript or couldn't parse correctly."}), 400
...
def delete_category(id):
    cat = db.get_or_404(CustomCategory, id)
...
@login_required
def delete_limit(id):
    lim = db.get_or_404(CategoryLimit, id)
...
def save_recurring():
    data = request.json
    if data.get('id'):
        r = db.session.get(RecurringExpense, data['id'])
...
def delete_recurring(id):
    r = db.get_or_404(RecurringExpense, id)
...
@app.route('/api/send-monthly-summary', methods=['POST'])
@login_required
def send_monthly_summary():
    """Compile this month's financial stats and email a styled summary to the user."""
    ...
    # ── This month's expenses ──────────────────────────────────────────────
    month_exps   = [e for e in user.expenses if e.date.startswith(curr_month)]
    ...
    top_cat_amt  = cat_sums.get(top_cat, 0)

    # Goals summary
    goals_on_track = sum(1 for g in user.goals if g.target_amount > 0 and (g.saved_amount / g.target_amount) >= 0.5)

    # Budget usage
    budget_pct   = round((total_spent / budget) * 100, 1) if budget > 0 else 0
    budget_color = '#ef4444' if budget_pct >= 100 else '#f59e0b' if budget_pct >= 80 else '#10b981'

    # Category rows HTML
    cat_rows_html = ''.join(
        f'<tr><td style="padding:6px 12px;color:#94a3b8;">{cat}</td>'
        f'<td style="padding:6px 12px;text-align:right;color:#e2e8f0;font-weight:600;">₹{amt:,.0f}</td></tr>'
        for cat, amt in sorted(cat_sums.items(), key=lambda x: x[1], reverse=True)
    ) if cat_sums else '<tr><td colspan="2" style="padding:6px 12px;color:#64748b;">No expenses this month.</td></tr>'

    html_body = f"""
    <div style="font-family:Inter,Arial,sans-serif;max-width:560px;margin:auto;background:#0f172a;color:#e2e8f0;border-radius:20px;overflow:hidden;">
      <!-- Header -->
      <div style="background:linear-gradient(135deg,#2563eb,#7c3aed);padding:32px 32px 24px;text-align:center;">
        <div style="font-size:28px;font-weight:900;color:#fff;letter-spacing:-0.5px;">🐦 Fin<span style="color:#bfdbfe;">Flap</span></div>
        <div style="font-size:13px;color:#bfdbfe;margin-top:4px;font-weight:500;">Monthly Financial Summary</div>
        <div style="font-size:22px;font-weight:700;color:#fff;margin-top:12px;">{month_label}</div>
      </div>
      <!-- Body -->
      <div style="padding:28px 32px;">
        <!-- Key Stats -->
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:24px;">
          <div style="background:#1e293b;border-radius:12px;padding:16px;border:1px solid rgba(255,255,255,0.07);">
            <div style="font-size:11px;color:#64748b;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px;">Total Spent</div>
            <div style="font-size:22px;font-weight:800;color:#f87171;">₹{total_spent:,.0f}</div>
          </div>
          <div style="background:#1e293b;border-radius:12px;padding:16px;border:1px solid rgba(255,255,255,0.07);">
            <div style="font-size:11px;color:#64748b;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px;">Savings</div>
            <div style="font-size:22px;font-weight:800;color:#34d399;">₹{max(savings, 0):,.0f}</div>
          </div>
          <div style="background:#1e293b;border-radius:12px;padding:16px;border:1px solid rgba(255,255,255,0.07);">
            <div style="font-size:11px;color:#64748b;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px;">Savings Rate</div>
            <div style="font-size:22px;font-weight:800;color:#818cf8;">{savings_rate}%</div>
          </div>
          <div style="background:#1e293b;border-radius:12px;padding:16px;border:1px solid rgba(255,255,255,0.07);">
            <div style="font-size:11px;color:#64748b;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px;">Budget Used</div>
            <div style="font-size:22px;font-weight:800;color:{budget_color};">{budget_pct}%</div>
          </div>
        </div>
        <!-- Category Breakdown -->
        <div style="background:#1e293b;border-radius:12px;padding:16px;margin-bottom:20px;border:1px solid rgba(255,255,255,0.07);">
          <div style="font-size:12px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:1px;margin-bottom:12px;">Spending by Category</div>
          <table style="width:100%;border-collapse:collapse;">{cat_rows_html}</table>
        </div>
        <!-- Top Insight -->
        <div style="background:linear-gradient(135deg,rgba(37,99,235,0.15),rgba(124,58,237,0.15));border:1px solid rgba(99,102,241,0.3);border-radius:12px;padding:16px;margin-bottom:20px;">
          <div style="font-size:13px;color:#a5b4fc;">🏆 Top Spending Category: <strong style="color:#fff;">{top_cat}</strong> — ₹{top_cat_amt:,.0f}</div>
          <div style="font-size:13px;color:#a5b4fc;margin-top:6px;">🎯 Goals on track (≥50%): <strong style="color:#fff;">{goals_on_track}</strong></div>
          <div style="font-size:13px;color:#a5b4fc;margin-top:6px;">📅 Monthly Income: <strong style="color:#fff;">₹{income:,.0f}</strong></div>
        </div>
        <!-- CTA -->
        <div style="text-align:center;margin-top:8px;">
          <a href="https://finflap.onrender.com" style="display:inline-block;padding:14px 32px;background:linear-gradient(135deg,#2563eb,#7c3aed);color:#fff;border-radius:12px;text-decoration:none;font-weight:700;font-size:14px;letter-spacing:0.3px;">Open FinFlap Dashboard →</a>
        </div>
      </div>
      <!-- Footer -->
      <div style="padding:16px 32px 24px;text-align:center;border-top:1px solid rgba(255,255,255,0.05);">
        <div style="font-size:11px;color:#334155;">FinFlap — Smart Finance Tracker &nbsp;|&nbsp; This is an automated summary for {user.email}</div>
      </div>
    </div>
    """
    ...
    try:
        msg = Message(
            subject=f'📊 FinFlap — Your {month_label} Summary',
            recipients=[user.email]
        )
        msg.html = html_body
        mail.send(msg)
        logger.info("Monthly summary sent to %s", user.email)
        return jsonify({'status': 'sent', 'message': f'Summary sent to {user.email}! 📬'})
    except Exception as e:
        logger.error("Failed to send monthly summary: %s", e)
